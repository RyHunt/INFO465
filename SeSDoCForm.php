...
if (trim($Locked) == 'Locked') {
  ...
  $UI = "<h2>Not Accepting Referrals Now</h2>\n\n<p>Referrals were locked $ModTime.</p>\n<p>They'll be unlocked when we feel like it.  Try later...</p>";
...
*/
  ...
  //Make check check boxes for MASEStatesVisited[] 
  //and radio buttons for FavSEState from file StatesSE
  //Make the groups in local variables to avoid looping with $TheForm
  $StatesSEFile = fopen('/home/SeSDoC/OptionsStatesSE','r');
  $VisitedStatesCB = '';
  $FavStateRB = '';
  while ($AState = fgets($StatesSEFile)) {
    $AState = trim($AState);
    $AStateNoSpaces = str_replace(' ','',$AState);  //Used to make a valid id with no spaces 
    //Only set 'checked' if the array is set from POST data, won't be for an empty form
    //Set 'checked' if the state is in_array, won't be for unchecked boxes
    if (isset($MASEStatesVisited) and $MASEStatesVisited != '' and in_array($AState, $MASEStatesVisited)) { 
      $CheckedSlug = 'checked'; 
    } else { 
      $CheckedSlug = ''; 
    }
    $VisitedStatesCB .= "       <label for=\"Visited$AStateNoSpaces\" class=\"WideLabel\">
  ...
  while ($AState = fgets($StatesSEFile)) {
    ...
    //Radio buttons return a scalar value, not in an array, set 'checked'
    //if this button holds the checked value
  ...
  //Put the fieldsets on the form
  $TheForm .= "
   <fieldset><legend>Southeastern States Visited</legend>
    $VisitedStatesCB
    </fieldset>
  ...
  $TheForm .= "     <div class=\"Col-12\">       
          <label for=\"MAOpinion\" class=\"WideLabel\">$SplatSlug What's your opinion of the Southeast?</label>      
            <textarea name=\"MAOpinion\" id=\"MAOpinion\" placeholder=\"Your opinion is very important to us as we consider your Referral...\">$MAOpinion</textarea>
        </div>
        </div>
      <div class=\"Row\"><br />";
  //Hard coded small select, put Options in $TheOptions to avoid thrashing $TheForm           
  $TheOptions = '';
  if ($MALeastFavoriteWeather == 'Heat') { $SelectedSlug = "selected"; } else { $SelectedSlug = ''; }
  $TheOptions .= "            <option value=\"Heat\" $SelectedSlug>Heat</option>\n";
  if ($MALeastFavoriteWeather == 'Humidity') { $SelectedSlug = "selected"; } else { $SelectedSlug = ''; }
  $TheOptions .= "             <option value=\"Humidity\" $SelectedSlug>Humidity</option>\n";
  if ($MALeastFavoriteWeather == 'Thunder') { $SelectedSlug = "selected"; } else { $SelectedSlug = ''; }
  $TheOptions .= "             <option value=\"Thunder\" $SelectedSlug>Thunder</option>\n";
  if ($MALeastFavoriteWeather == 'Rain') { $SelectedSlug = "selected"; } else { $SelectedSlug = ''; }
  $TheOptions .= "             <option value=\"Rain\" $SelectedSlug>Rain</option>\n";
  if ($MALeastFavoriteWeather == 'Snow') { $SelectedSlug = "selected"; } else { $SelectedSlug = ''; }
  $TheOptions .= "             <option value=\"Snow\" $SelectedSlug>Snow</option>";
  ...
  //Make options for multi-select using contents of text file with Options...
  $TheOptions = '';
  $StatesNotSEFile = fopen('/home/SeSDoC/OptionsStatesNotSE','r');
  while ($AState = fgets($StatesNotSEFile)) {
    $AState = trim($AState);
    //$AStateNoSpaces = str_replace(' ','',$AState);
    if (isset($MAOtherStatesVisited) and $MAOtherStatesVisited != '' and in_array($AState, $MAOtherStatesVisited)) {
...
if ($View == 'First') {
  ...
  $UI = "  <h2>SeSDoC Membership Referral</h2>
  ...
  if (!isset($MASEStateFavorite) or $MASEStateFavorite  == '') $ValidationErrors['MASEStateFavorite'] = "You must select your favorite Southeastern state to have your Referral considered.";
/*
